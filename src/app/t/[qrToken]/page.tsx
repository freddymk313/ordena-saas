...
export default function ClientTablePage({
  ...
  const handleTriggerServiceRequest = async (type: "call_server" | "request_bill") => {
    ...
    try {
      ...
      if (res.ok) {
        setServiceNotice(
          type === "call_server"
            ? "🔔 Un serveur a été prévenu et arrive !"
  ...
  if (loadingSession) {
    return (
      <div className="min-h-dvh bg-gray-50 flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-xs text-gray-400 mt-1">Chargement du menu en direct</p>
      </div>
    );
  }
  ...
  if (sessionError || !table || !tenant) {
    return (
      <div className="min-h-dvh bg-gray-50 flex flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-xl font-bold text-gray-900 mb-2">Session non trouvée</h1>
  ...
  return (
    <div className="min-h-dvh bg-gray-50/80 text-gray-900 flex flex-col pb-36 font-sans max-w-md mx-auto relative shadow-2xl min-w-[320px] select-none">
      ...
      <AnimatePresence>
        {serviceNotice && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed top-4 left-4 right-4 z-50 max-w-md mx-auto bg-gray-900 text-white p-3.5 rounded-2xl shadow-xl flex items-center justify-between text-xs font-medium border border-gray-800"
          >
            <span>{serviceNotice}</span>
            <button onClick={() => setServiceNotice(null)} className="text-gray-400 hover:text-white p-1">
              <X className="w-4 h-4" />
            </button>
          </motion.div>
        )}
      ...
      {/* Brand Header Banner */}
      <header className="sticky top-0 z-30 px-4 py-5 bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-2xs">
        <div className="flex items-center justify-between gap-3">
          {/* Brand & Table Label */}
          <div className="flex items-center gap-3 min-w-0">
            <button
              onClick={() => setActiveView("menu")}
              aria-label="Voir le menu du restaurant"
            ...
            <div className="min-w-0">
              <h1 className="font-extrabold text-[13px] text-gray-900 truncate leading-tight">
                {tenant.name}
              </h1>
          ...
          {/* Action Icons (Store & Bell) */}
          <div className="flex items-center gap-2 shrink-0">
            {/* Notification / Order Status Bell Button */}
            <button
              onClick={() =>
                setActiveView(activeView === "menu" ? "order_status" : "menu")
              }
        ...
        <main className="flex-1 p-4 space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-white border border-gray-200 rounded-2xl text-xs font-medium placeholder-gray-400 focus:outline-hidden focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 shadow-2xs transition-all"
            />
          </div>

          {/* Categories Scroll */}
          <div className="flex items-center gap-2 overflow-x-auto pb-1 touch-pan-x snap-x scrollbar-none">
            <button
              onClick={() => setSelectedCategory("all")}
              className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap snap-start transition-all ${
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-2.5 rounded-xl text-xs font-bold whitespace-nowrap snap-start transition-all ${
            ...
            {filteredItems.length > 0 ? (
              filteredItems.map((item) => {
                ...
                return (
                  <div
                    key={item._id}
                    className="bg-white rounded-2xl p-3 border border-gray-100 shadow-2xs flex gap-3 items-center justify-between"
                  >
                    {/* Item Photo */}
                    <div className="w-20 h-20 rounded-xl bg-gray-100 overflow-hidden shrink-0 relative border border-gray-50">
                      {item.photoUrl ? (
                    ...
                    <div className="flex-1 min-w-0 pr-1">
                      <h3 className="font-bold text-gray-900 text-xs sm:text-sm leading-snug truncate">
                        {item.name}
                      </h3>
      ...
      {activeView === "order_status" && (
        <main className="flex-1 p-4 space-y-4">
          ...
          <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-2xs space-y-3">
            <h3 className="font-bold text-xs text-gray-900">Détail de votre commande</h3>
            <div className="divide-y divide-gray-100">
              {orders.flatMap((o) => o.items).map((item, idx) => (
                <div key={idx} className="py-2.5 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2 min-w-0 pr-2">
                    <span className="w-5 h-5 rounded-md bg-gray-100 font-bold text-gray-800 flex items-center justify-center shrink-0 text-[11px]">
                      {item.quantity}x
                    </span>
            ...
            <div className="pt-2 border-t border-gray-100 flex items-center justify-between font-extrabold text-xs sm:text-sm text-gray-900">
      ...
      {activeView === "rating" && (
        <main className="flex-1 p-4 space-y-4">
          <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-2xs space-y-2 text-center">
            ...
            <p className="text-xs text-gray-500">
              Votre addition a été réglée. Prenez un instant pour noter votre repas chez{" "}
              <strong className="text-gray-900">{tenant.name}</strong>.
            </p>
          </div>

          <form onSubmit={handleSubmitRatings} className="space-y-3">
            {Array.from(
              new Map(
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[11px] text-gray-400">Comment était ce plat ?</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-center gap-2 pt-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <Star
                          className={`w-7 h-7 ${
                            star <= currentRating ? "fill-amber-400 text-amber-400" : "text-gray-200"
                          }`}
                        />
                      </button>
                    ))}
            ...
            <button
              ...
              className="w-full py-3.5 px-4 rounded-xl text-white font-extrabold text-xs sm:text-sm shadow-md transition-transform active:scale-98 flex items-center justify-center gap-2"
              style={{ backgroundColor: brandColor }}
            >
      ...
      {activeView === "rated_thank_you" && (
        <main className="flex-1 p-6 flex flex-col items-center justify-center text-center space-y-4">
          ...
          <p className="text-xs text-gray-500 max-w-xs leading-relaxed">
            Vos retours nous aident à améliorer continuellement la qualité de notre service. À très bientôt !
          </p>
        </main>
      )}
      ...
      {activeView === "menu" && cartItemsCount > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-40 p-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] bg-white/95 backdrop-blur-md border-t border-gray-200/80 shadow-lg max-w-md mx-auto">
          <button
            ...
            className="w-full py-3.5 px-4 rounded-2xl text-white font-extrabold text-xs sm:text-sm shadow-md flex items-center justify-between transition-transform active:scale-98"
            style={{ backgroundColor: brandColor }}
          >
        ...
        {isCartOpen && (
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-end justify-center">
            <motion.div
              ...
              className="bg-white rounded-t-3xl w-full max-w-md p-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] space-y-4 shadow-2xl max-h-[90dvh] flex flex-col"
            >
              {/* Modal Drag Handle */}
              <div className="w-12 h-1 bg-gray-200 rounded-full mx-auto shrink-0 mb-1" />
              ...
              <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                ...
                <button
                  ...
                  className="p-1 text-gray-400 hover:text-gray-600 rounded-lg active:scale-90 transition-transform"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="overflow-y-auto flex-1 space-y-2.5 pr-1">
                {Object.entries(cart).map(([itemId, qty]) => {
                  ...
                  return (
                    <div
                      key={itemId}
                      className="p-3 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-between gap-2 text-xs"
                    >
                      <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center bg-white rounded-xl p-1 gap-1 border border-gray-200 shrink-0">
                        <button
                          onClick={() => updateQuantity(itemId, -1)}
                          className="w-7 h-7 rounded-lg bg-gray-100 text-gray-800 flex items-center justify-center font-bold active:scale-90"
                        >
                          <Minus className="w-3.5 h-3.5" />
                        </button>
                        <span className="font-bold min-w-[16px] text-center">{qty}</span>
                        <button
                          onClick={() => updateQuantity(itemId, 1)}
                          className="w-7 h-7 rounded-lg text-white flex items-center justify-center font-bold active:scale-90"
                          style={{ backgroundColor: brandColor }}
                        >
                          <Plus className="w-3.5 h-3.5" />
                        </button>
              ...
              <form onSubmit={handlePlaceOrder} className="space-y-3 pt-2 border-t border-gray-100">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-3 bg-gray-50 border border-gray-200 rounded-xl text-xs font-semibold focus:outline-hidden focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full py-3.5 px-4 rounded-2xl text-white font-extrabold text-xs sm:text-sm shadow-md transition-transform active:scale-98 flex items-center justify-center gap-2"
                  style={{ backgroundColor: brandColor }}
                >
...
function OrderCountdownCard({
  ...
  return (
    <div className="bg-emerald-50/60 rounded-xl p-3.5 border border-emerald-100 space-y-2.5">
      ...
      {steps.map((step, idx) => {
        ...
        return (
          <div key={step.key} className="flex flex-col items-center text-center space-y-1">
            <div
              className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center font-bold text-[10px] sm:text-xs transition-all ${
                isCurrent
